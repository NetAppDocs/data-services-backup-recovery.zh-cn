...
---
= 在 NetApp Backup and Recovery 中规划 ONTAP 卷保护
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用此页面可规划如何使用 NetApp Backup and Recovery 保护 ONTAP 卷。在启用备份和恢复之前，请查看保护方法、架构、策略、对象存储选择和 Console 代理部署模式。
...
在选择要使用的功能之前，以下是每项功能的简要说明及其提供的保护类型。
...
|===
快照是所有备份方法的基础，使用备份和恢复服务需要用到快照。快照是卷的只读时间点映像。该映像占用的存储空间极少，性能开销可以忽略不计，因为它只记录自上次创建快照以来对文件所做的更改。在卷上创建的快照用于使复制的卷和备份文件与对源卷所做的更改保持同步，如图所示。

image:diagram-321-overview.png["一张图表显示了备份文件在源系统中以快照形式存在，在辅助存储系统中以复制卷形式存在，以及在对象存储中以备份文件形式存在。"]
...
除了这些标准之外，如果您使用 *DataLock 和勒索软件防护* 功能，云存储还提供了额外的安全选项，当您为较旧的备份文件选择归档存储类时，还可以节省额外的成本。link:prev-ontap-policy-object-options.html["了解有关 DataLock 和勒索软件保护以及档案存储设置的更多信息"]。
...
image:diagram-321-fanout-detailed.png["图示说明如何使用卷的快照来创建和更新复制卷和备份文件。"]

级联架构首先将快照传输到目标存储系统，然后该系统将副本传输到云端的备份对象。

image:diagram-321-cascade-detailed.png["图示说明如何使用卷的快照来创建和更新复制卷和备份文件。"]
...
您可以使用 NetApp 提供的默认策略创建备份，也可以创建自定义策略。使用激活向导为卷启用备份和恢复服务时，可以从默认策略和系统中已存在的任何其他策略（Cloud Volumes ONTAP 或本地 ONTAP 系统）中进行选择。如果要使用与现有策略不同的策略，可以在启动激活向导之前或使用激活向导时创建策略。
...
您可以在 NetApp Backup and Recovery UI 中创建 Snapshot、复制和备份到对象存储策略。有关详细信息，请参见 link:prev-ontap-backup-manage.html["添加新的备份策略"]。

除了使用 NetApp Backup and Recovery 创建自定义策略外，还可以使用 System Manager 或 ONTAP 命令行界面 (CLI)：
...
备份策略驻留在不同的位置，具体取决于您是使用扇出式架构还是级联式架构。复制策略和备份策略的设计不同，因为复制将两个 ONTAP 存储系统配对，而备份到对象使用存储提供程序作为目标。
...
| *级联* | 主云 | 二级 | 二级 
|===
因此，如果您计划在使用级联架构时创建自定义策略，则需要在将创建复制卷的二级系统上创建复制和备份到对象策略。如果您计划在使用扇出架构时创建自定义策略，则需要在将创建复制卷的二级系统上创建复制策略，并在主系统上创建备份到对象策略。
...
在系统的对象存储中创建备份文件时，默认情况下，备份和恢复服务会为已配置的对象存储帐户中的备份文件创建容器（存储区或存储帐户）。AWS 或 GCP 存储区默认命名为 "netapp-backup-<uuid>"。Azure Blob 存储帐户命名为 "netappbackup<uuid>"。
...
NOTE: 在 Azure 中创建用作备份目标的容器时，请确保创建的容器具有与 Azure 存储帐户相同的名称。
...
如果已在使用控制台管理存储，则已安装控制台代理。如果您计划将同一个控制台代理与 NetApp Backup and Recovery 一起使用，则一切就绪。如果需要使用其他控制台代理，请在启动备份和恢复实施之前安装它。
...
在具有完整互联网连接的站点（也称为_标准模式_或_SaaS 模式_）中使用 NetApp Backup and Recovery 时，可以在 Console 管理的任何本地 ONTAP 或 Cloud Volumes ONTAP 系统上创建复制卷，并可以在任何受支持的云提供商的对象存储中创建备份文件。link:concept-backup-to-cloud.html["查看受支持的备份目标的完整列表"]。
...
* 您可以将数据从本地 ONTAP 系统备份到本地 StorageGRID 系统。link:prev-ontap-backup-onprem-storagegrid.html["将本地ONTAP数据备份到StorageGRID"]。
* 您可以将数据从本地ONTAP系统备份到本地ONTAP系统或为 S3 对象存储配置的Cloud Volumes ONTAP系统。link:prev-ontap-backup-onprem-ontaps3.html["将本地ONTAP数据备份到ONTAP S3"]。
