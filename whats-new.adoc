...
link:prev-ontap-backup-manage.html["了解更多关于使用NetApp Backup and Recovery备份ONTAP卷数据的信息"]。
...
* NetApp Backup and Recovery现已正式提供 VMware 工作负载支持 (GA)。
* 您现在可以恢复客户操作系统文件和文件夹。


https://docs.netapp.com/us-en/data-services-backup-recovery/br-use-vmware-protect-overview.html["了解更多关于使用NetApp Backup and Recovery恢复 VMware 虚拟机的信息"]。
...
* 现在您可以将 Hyper-V VM 备份和快照还原到备用位置。如果您想管理不同 Hyper-V 主机上的虚拟机版本，这将非常有用。
* NetApp Backup and Recovery现在支持由 System Center Virtual Machine Manager (SCVMM) 配置并托管在 CIFS 共享上的 Hyper-V 虚拟机。
* 您现在可以编辑保护组了。
...
KVM 工作负载预览版现在支持与 Apache CloudStack 管理平台集成，使您能够保护由 Apache CloudStack 管理的 KVM 主机和虚拟机。
