...
[role="lead"]
发现或手动添加异地备份目标，例如 AWS S3、Azure Blob Storage、Google Cloud Storage 或 StorageGRID，以便您可以将备份存储在主环境之外。
...
在使用 NetApp Backup and Recovery 之前，请先配置备份目标（Amazon Web Services (AWS) S3、Microsoft Azure Blob Storage、Google Cloud Storage 或 StorageGRID）。

您可以自动发现这些目标或手动添加它们。

提供访问存储帐户的凭据。NetApp Backup and Recovery 使用这些凭据来发现和验证对备份目标存储桶的访问。

.开始之前
您需要先发现至少一个工作负载，然后才能添加异地备份目标。

.步骤
. 在 NetApp Backup and Recovery 菜单中，选择*备份目标*。
. 选择*发现备份目标*。
. 从可用的备份目标类型中选择一种：*AWS*、*Azure*、*StorageGRID*、*ONTAP S3* 或 *GCP*。
. 选择“下一步”。
. （仅适用于 AWS 和 Azure 备份目标）在 *选择凭据位置* 部分，选择凭据所在的位置，然后选择如何关联凭据。
. 选择“下一步”。
...
*** *应用程序（客户端）ID*：输入 Azure Blob 存储应用程序 ID。
...
*** *网关节点 FQDN*：输入 StorageGRID 网关节点的 FQDN。
...
*** *网关节点 FQDN*：输入 ONTAP S3 网关节点的 FQDN。
*** *端口*：输入ONTAP S3 的端口号。
...
** 对于 GCP：
+
*** *凭据名称*：输入 Google Cloud 凭据名称。
*** *访问密钥*：输入 Google Cloud 访问密钥。
*** *密钥*：输入 Google Cloud 密钥。
...
手动将存储桶添加到异地备份目标，而不是让 NetApp Backup and Recovery 自动发现存储桶。

.步骤
. 在 NetApp Backup and Recovery 菜单中，选择*备份目标*。
. 选择一个目标，然后在右侧选择*操作*image:icon-action.png["操作图标"]图标，然后选择*添加存储桶*。
...
*** *实例详情*：
+
**** *存储帐户名*：输入 Azure Blob 存储帐户的名称。
**** *Azure region*：输入存储帐户的 Azure 区域。
**** *性能类型*：为 Azure Blob 存储帐户选择标准或高级的性能类型，指示所需的性能级别。
**** *加密*：选择 Azure Blob 存储帐户的加密类型。选项可以是 Microsoft 托管的密钥，也可以是客户托管的密钥。如果您选择客户托管的密钥，则必须提供密钥保管库名称和密钥名称。
...
** 对于ONTAP S3：
+
*** *备份目标*：选择 ONTAP S3 备份目标的名称。
*** *存储桶名称*：输入 ONTAP S3 存储桶的名称。
...
** 对于 GCP：
+
*** *存储桶名称*：输入 Google Cloud 存储桶的名称。
*** *GCP 项目*：选择 Google Cloud 项目的名称。
*** *存储桶区域*：为 Google Cloud 存储桶选择一个区域。
*** *对象锁定*：选择此选项可启用此存储桶中对象的保留期。在保留期有效期间，无法修改或删除对象，并且在期限到期之前不可变。如果为存储桶启用此选项，则无法将其关闭。
...
.步骤
. 在 NetApp Backup and Recovery 菜单中，选择*备份目标*。
. 选择一个目标，然后在右侧选择*操作*image:icon-action.png["操作图标"]图标并选择*更改凭据*。
