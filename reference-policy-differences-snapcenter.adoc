...
---
= SnapCenter与NetApp Backup and Recovery之间的策略差异
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
使用此参考了解 SnapCenter 与 NetApp Backup and Recovery 之间可能影响导入资源和策略的策略差异。
...
NetApp Backup and Recovery 按如下方式调整 SnapCenter 计划：
...
* 如果 SnapCenter 计划设置为大于 7 天，则 NetApp Backup and Recovery 会将计划设置为每月一次。本月将跳过一些快照。
+
...
*示例*：如果您有 SnapCenter 每小时策略，从凌晨 1:00 开始每 5 小时重复一次，NetApp Backup and Recovery（迁移后）会将计划设置为每日，5 小时间隔为凌晨 1:00、上午 6:00、上午 11:00、下午 4:00 和晚上 9:00。某些时间将被跳过——晚上 9:00 之后，每 5 小时重复一次应该是凌晨 2:00，但始终是凌晨 1:00。
