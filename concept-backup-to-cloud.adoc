...
服务费支付给NetApp ，涵盖创建备份到对象存储或ONTAP目标的成本，以及从这些备份恢复卷或文件的成本。您只需为在对象存储或ONTAP中保护的数据付费（这包括SnapMirror配置）。费用是根据备份到对象存储或ONTAP目标的ONTAP卷的源逻辑已用容量（在ONTAP效率之前）计算的。此容量也称为前端兆字节 (FETB)。
