...
* *VMware 工作负载（使用不含 SnapCenter Plug-in for VMware vSphere 的新 UI）*：
+
** 使用NetApp Backup and Recovery保护您的 VMware VM 和数据存储。
** 将 VMware 工作负载备份到 Amazon Web Services S3 或 StorageGRID。
...
* Microsoft Azure Blob（不适用于 VMware 工作负载）
* StorageGRID
* ONTAP S3（不适用于 VMware 工作负载）
