...
[role="lead"]
使用此信息为 NetApp Backup and Recovery 备份选择 AWS S3 存档层。您还可以查看已存档备份的恢复成本和检索时间。
...
最初创建备份文件时，它们存储在 S3 _Standard_ 存储中。此层针对存储不常访问的数据进行了优化，但仍允许即时访问。30 天后，备份将转换为 S3 _Standard-Infrequent Access_ 存储类以节省成本。
