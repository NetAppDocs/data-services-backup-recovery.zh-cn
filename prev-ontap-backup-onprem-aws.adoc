...
[role="lead"]
在 NetApp Backup and Recovery 中，可以将卷数据从本地 ONTAP 系统备份到二级存储系统和 Amazon S3。使用此页面可准备您的环境并激活备份。
...
NOTE: 客户管理的密钥加密仅支持 Amazon FSx for NetApp ONTAP。Cloud Volumes ONTAP for AWS 不支持此功能。

请参阅 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["在 AWS 中设置客户管理的密钥加密"^]。
