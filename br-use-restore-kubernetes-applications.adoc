...
---
= 使用 NetApp Backup and Recovery Web UI 还原 Kubernetes 应用程序
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
在 NetApp Backup and Recovery 中还原受保护策略保护的应用程序。要还原应用程序，至少需要一个还原点，其中可以包括本地快照、对象存储备份或两者。您可以从本地、辅助或对象存储副本还原。
...
+
还原数据向导打开，并显示_常规设置_页面。

. 选择要从中还原的源位置。
. 从*Cluster*列表中选择目标集群。
. 选择还原到原始命名空间或新命名空间。
. 如果选择还原到新命名空间，请执行以下操作：
+
.. 输入要使用的目标命名空间。
.. 输入目标应用程序名称。
.. （可选）选择*不为还原的资源创建应用程序*选项，在不创建应用程序自定义资源对象的情况下还原资源。这减少了应用程序清单中不必要的条目。
...
.. 执行以下操作之一：
+
*** 要恢复整个虚拟机，请选择*虚拟机*选项卡。
+
Backup and Recovery 列出了恢复点中的所有虚拟机。您可以选择要包含在还原操作中的虚拟机。

*** 要恢复单个持久卷声明，请选择*持久卷声明*选项卡。
+
Backup and Recovery 列出了恢复点中的所有永久卷声明。您可以选择要在还原操作中包含的永久卷声明。
...
. 或者，如果您要还原使用 Trident Protect 创建的备份或快照，请查看用作还原操作存储桶的 AppVault 的详细信息。如果您的环境发生变化或 AppVault 状态发生变化，请选择 *Sync App Vault* 以刷新详细信息。
+
...
NOTE: 在全局 link:br-start-configure.html#create-a-resource-transformation-template["设置"^] 区域创建资源转换模板。
