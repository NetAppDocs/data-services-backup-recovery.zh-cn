...
* <<为主机资源添加凭据>> 适用于任何 Windows、Microsoft SQL Server、Oracle Database 或 Linux 主机，NetApp Backup and Recovery 需要对其进行身份验证。这包括在还原文件或文件夹时使用的来宾操作系统凭据。
...
. 选择“*凭证*”的向下箭头。
. 选择*添加新凭证*。
. 请输入登录凭证信息。根据您选择的身份验证模式，会显示不同的字段。将鼠标悬停在信息 *i* 图标上，可查看有关字段的更多信息。
+
** *凭证名称*：输入凭证的名称。
** *身份验证模式*：选择 *Windows*、*Microsoft SQL*、*Oracle Database* 或 *Linux*。
+

TIP: 对于 Microsoft SQL Server 工作负载，您需要输入 Windows 和 Microsoft SQL Server 的凭据，因此您需要添加两组凭据。
...
如果您使用 Apache CloudStack 管理平台来管理 KVM 资源，则需要将其与NetApp Backup and Recovery集成，以便 Backup and Recovery 可以发现和保护受管 KVM 主机和虚拟机。
...
创建资源转换模板，以便在还原资源时存储对资源进行的常见转换。

.步骤
. 从NetApp Backup and Recovery菜单中，选择 *设置*。
. 选择向下箭头以展开 *Resource modifer template* 部分。
. 选择 *Create template*。
. 输入资源转换模板的名称。
. （可选）输入模板说明。
. 描述目标资源：
