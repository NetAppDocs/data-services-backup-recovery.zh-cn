...
[role="lead"]
在使用 SnapMirror 逻辑复制 (LRSE) 迁移卷后，在 NetApp Backup and Recovery 中使用 SnapMirror to Cloud Resync，以保持现有云备份可用。此功能允许备份在无需重新基线的情况下继续进行，并从 ONTAP 9.16.1 开始提供。

SnapMirror to Cloud Resync 支持 FlexVol 和 FlexGroup 迁移，并帮助您在混合云和多云环境中保持备份连续性。
...
假设您有一个名为 Vol1a 的本地卷。此卷有三个快照：S1、S2 和 S3。这些快照是还原点。Vol1a 使用 SnapMirror to Cloud (SM-C) 备份到云，但只有 S1 和 S2 在对象存储中。
...
NOTE: 请勿使用"Vault"类型的 SnapMirror 策略将卷从旧 ONTAP 群集克隆到新 ONTAP 群集或在级联架构设置中。Vault 策略在传输过程中更改快照的版本 UUID。快照的版本 UUID 用于确定源和目标之间是否存在公共快照。
...
. 选择*提交*开始验证过程。
+
NetApp Backup and Recovery 验证所选用于重新同步的每个卷是否与对象存储共享一个通用快照。具体来说，存储在对象存储中的最新快照也必须存在于新克隆卷的本地快照中。此检查确保卷已准备好进行 SnapMirror 到云端重新同步操作。

. 查看验证结果，包括新的源卷名称和每个卷的重新同步状态。
. 检查卷的资格。系统检查卷是否符合重新同步的资格。如果卷不符合条件，则表示不符合通用快照条件。
+
