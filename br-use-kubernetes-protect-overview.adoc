...
[role="lead"]
通过在NetApp Backup and Recovery 中管理 Kubernetes 工作负载，您可以在一个地方发现、管理和保护您的 Kubernetes 集群和应用程序。您可以管理托管在 Kubernetes 集群上的资源和应用程序。您还可以创建保护策略并将其与 Kubernetes 工作负载关联，所有这些都使用单一界面完成。
...
NetApp Backup and Recovery 为管理 Kubernetes 工作负载提供了以下优势：
