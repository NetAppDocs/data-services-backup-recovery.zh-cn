...
有关 StorageGRID 的 Datalock 和 Ransomware 保护要求的详细信息，请参阅 link:prev-ontap-policy-object-options.html["备份到对象策略选项"]。
...
+
如果您的集群使用的是 ONTAP 9.11.1 或更高版本，则可以通过配置 _Datalock and Ransomware protection_ 来选择保护备份免受删除和勒索软件攻击。_DataLock_ 保护您的备份文件不被修改或删除，_ransomware protection_ 扫描您的备份文件，在备份文件中查找勒索软件攻击的证据。
