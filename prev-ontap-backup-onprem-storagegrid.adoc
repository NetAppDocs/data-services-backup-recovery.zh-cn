...
[role="lead"]
在 NetApp Backup and Recovery 中，可以将卷数据从主本地 ONTAP 系统备份到二级存储系统和 StorageGRID 对象存储。使用此页面可准备您的环境并激活备份。
...
+
新版本的 NetApp Backup and Recovery 包括计划和创建快照及复制卷以及创建对象存储备份的功能，要求您使用 3.9.31 或更高版本的 Console 代理。NetApp 建议您使用此最新版本来管理所有备份。
...
要了解有关 StorageGRID 的 DataLock 和勒索软件防护要求的详细信息，请参阅 link:prev-ontap-policy-object-options.html["备份到对象策略选项"]。

支持的StorageGRID版本:: 支持StorageGRID 10.3 及更高版本。
+
--
要使用 DataLock & Ransomware 防护进行备份，您的 StorageGRID 系统必须运行 11.6.0.3 或更高版本。

要将旧备份分层到云档案存储，您的StorageGRID系统必须运行 11.3 或更高版本。此外，您的StorageGRID系统必须在控制台*系统*页面上被发现。

要使用归档存储，需要管理节点 IP 访问。
...
+
如果您的集群使用的是 ONTAP 9.11.1 或更高版本，则可以通过配置 _DataLock 和勒索软件保护_ 来选择保护备份免受删除和勒索软件攻击。_DataLock_ 保护您的备份文件不被修改或删除，_ransomware protection_ 扫描您的备份文件，在备份文件中查找勒索软件攻击的证据。
