...
[role="lead"]
使用此信息了解 NetApp Backup and Recovery 支持的 Azure 存档层。您还可以查看已存档备份的恢复成本和检索时间。
...
最初创建备份文件时，它们将存储在 _Cool_ 访问层中。此层针对存储不常访问的数据进行了优化，但可以在需要时立即访问。
...
NOTE: 将数据从 Azure 还原到 StorageGRID 系统时，不支持高优先级。
