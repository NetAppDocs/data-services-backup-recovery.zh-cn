...
[role="lead"]
在 NetApp Backup and Recovery 中，可以将卷数据从本地 ONTAP 系统备份到二级存储系统和 Azure Blob。使用此页面可准备您的环境并激活备份。
...
+
当控制台代理部署在云中时，Azure 政府区域支持 NetApp Backup and Recovery，而不是在本地安装时。此外，必须从 Azure Marketplace 部署控制台代理。您无法从 NetApp Console SaaS 网站在政府区域中部署控制台代理。
...
+
** 对于 NetApp Backup and Recovery PAYGO 许可，您需要订阅 https://marketplace.microsoft.com/en-us/product/netapp.cloud-manager?tab=Overview["Azure 市场提供的NetApp Console"^]。NetApp Backup and Recovery 的计费通过此订阅完成。
...
* 如果使用的 IPspace 与默认 IPspace 不同，则可能需要创建静态路由才能访问对象存储。
...
+
请注意，NetApp Backup and Recovery 支持 _Azure 访问策略_ 作为权限模型。当前不支持 _Azure 基于角色的访问控制_ (Azure RBAC) 权限模型。
...
+
** 在 Console *Systems* 页面中，选择系统，然后选择右侧面板中备份和恢复服务旁边的 *Enable > Backup Volumes*。
+
...
+
向导的"简介"页面显示保护选项，包括本地快照、复制和备份。如果在此步骤中执行了第二个选项，则会显示"定义备份策略"页面，其中选择了一个卷。
...
NOTE: 如果选择 *自带存储桶*，请确保创建作为备份目标的容器与 Azure 存储帐户同名。
