...
---
= 在还原期间修改 Kubernetes 资源（NetApp Backup and Recovery）
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
在恢复操作期间，使用资源转换来修改 Kubernetes 资源。有关在 Web UI 中修改资源的信息，请参阅 link:br-use-restore-kubernetes-applications.html#restore-kubernetes-applications["还原期间修改资源"^]。

*必需的 NetApp Console 角色* 备份和恢复超级管理员或备份和恢复还原管理员。如果要保护存储在 Cloud Volumes ONTAP 集群上的数据，则还需要 Cloud Volumes ONTAP 管理员角色。link:reference-roles.html["了解NetApp Backup and Recovery访问角色"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["了解所有服务的NetApp Console访问角色"^].
...
使用该 `replace`操作替换指定路径下资源中的现有值。JSON Pointer 路径必须已存在。以下示例更改 Route 对象的主机名：
