...
[role="lead"]
在 NetApp Backup and Recovery 中，您可以将卷数据从本地 ONTAP 系统备份到二级存储系统和 Google Cloud Storage。使用此页面可准备您的环境并激活备份。
...
* 请注意，如果您使用的 IPspace 与 Default 不同，则可能需要创建静态路由以访问对象存储。
...
+
如果备份的 Google Cloud Storage 目标存在于 Console *Systems* 页面上，您可以将 ONTAP 集群拖动到 Google Cloud Storage 上。
...
** *加密密钥*：如果您创建了新的 Google Cloud Storage 存储桶，请输入提供商提供的加密密钥信息。选择是使用默认的 Google Cloud 加密密钥，还是从 Google Cloud 帐户中选择自己的客户管理密钥来管理数据加密。
+

NOTE: 如果您选择了现有的 Google Cloud Storage 存储桶，则加密信息已可用，因此您无需立即输入。
