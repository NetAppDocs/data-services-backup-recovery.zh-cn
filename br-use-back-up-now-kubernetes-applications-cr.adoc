...
summary: '使用自定义资源 (CR) 创建 Kubernetes 应用程序的按需快照和备份，建立基线或保护您最近的数据更改。' 
---
= 立即使用 NetApp Backup and Recovery 中的自定义资源备份 Kubernetes 应用程序
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用自定义资源 (CR) 创建 Kubernetes 应用程序的按需快照和备份，建立基线或保护您最近的数据更改。
...
** *spec.reclaimPolicy*：（_Optional_）定义删除快照 CR 时快照的 AppArchive 会发生什么。即使设置为 `Retain`，快照 CR 本身仍会被删除；仅保留 AppArchive。有效选项：
+
*** `Retain` (默认)
*** `Delete`
+
...
** *spec.dataMover*：（_可选，从 Trident Protect 迁移的集群必需_）表示要用于备份操作的备份工具的字符串。如果此集群已从 Trident Protect 迁移到 Backup and Recovery，则该值区分大小写，并且必须为 `CBS`。
