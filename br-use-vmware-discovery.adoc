...
[role="lead"]
在保护运行在 ONTAP 系统上的 VMware 数据存储和虚拟机之前，您需要在 NetApp Backup and Recovery 中发现它们。如果您已经安装了 SnapCenter Plug-in for VMware vSphere，还可以导入其备份数据和策略。
...
如果您尚未安装 SnapCenter Plug-in for VMware，请添加 vCenter 信息，以便 NetApp Backup and Recovery 可以发现工作负载。在每个 Console 代理中，选择要发现工作负载的系统。
...
如果您已经安装了适用于 VMware 的 SnapCenter 插件，请使用以下步骤将 SnapCenter 插件资源导入到 NetApp Backup and Recovery 中。Console 从插件中发现 ESXi 主机、数据存储区、虚拟机和计划，因此您无需重新创建该信息。
...
NOTE: 如果 SnapCenter 插件已注册，您可以更新现有的注册详细信息。
