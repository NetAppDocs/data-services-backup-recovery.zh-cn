...
---
= 使用 NetApp Backup and Recovery 将 Cloud Volumes ONTAP 备份到 Azure Blob 存储
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用 NetApp Backup and Recovery 将卷数据从 Cloud Volumes ONTAP 系统备份到 Azure Blob 存储。
...
若要使用 NetApp Backup and Recovery 搜索和还原功能，控制台代理的角色必须包含特定权限，以便可以访问 Azure Synapse 工作区和 Azure Data Lake Storage 帐户。请参阅以下权限，如果您需要修改策略，请按照以下步骤操作。

.开始之前
* 您必须在订阅中注册 Azure Synapse Analytics 资源提供程序（称为"Microsoft.Synapse"） https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/resource-providers-and-types#register-resource-provider["了解如何为您的订阅注册此资源提供程序"^]。您必须是订阅 *Owner* 或 *Contributor* 才能注册资源提供商。
...
您可以在新的或现有的 Cloud Volumes ONTAP 系统上启用 NetApp Backup and Recovery。每个选项的步骤略有不同。
...
.步骤
. 从 Console *Systems* 页面中，选择系统，然后选择右侧面板中 Backup and Recovery 旁边的*启用*。
+
...
随时直接从 Cloud Volumes ONTAP 系统激活备份。
...
+
向导的"简介"页面显示保护选项，包括本地快照、复制和备份。如果在此步骤中使用了第二个选项，则会显示"定义备份策略"页面，其中选择了一个卷。
...
选择要保护的卷。受保护的卷具有以下一项或多项：快照策略、复制策略、备份到对象策略。
...
NOTE: 如果选择了现有 Azure 存储帐户，则加密信息已可用，因此无需立即输入。
...
.步骤
. 在“审核”页面中，审核您的选择。
. （可选）选中该框以*自动将快照策略标签与复制和备份策略标签同步*。这将创建具有与复制和备份策略中的标签匹配的标签的快照。
. 选择*激活备份*。


.结果
NetApp Backup and Recovery开始对您的卷进行初始备份。复制卷和备份文件的基线传输包括主存储系统数据的完整副本。后续传输包含快照中包含的主存储数据的差异副本。
...
此时会出现 Volume Backup Dashboard（卷备份仪表板），您可以监控备份的状态。
