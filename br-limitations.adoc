...
SnapCenter 没有将存储集群或存储 VM 映射到资源所在主机的功能，但 NetApp Backup and Recovery 有。在 NetApp Backup and Recovery 预览版本中，本地 ONTAP 集群或存储 VM 将不会映射到主机。此外，NetApp Console 不支持存储 VM。
