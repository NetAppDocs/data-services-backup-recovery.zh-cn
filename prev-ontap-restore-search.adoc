...
---
= 使用 NetApp Backup and Recovery 中的搜索和还原功能还原 ONTAP 数据
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
当您不知道确切的系统、卷或文件名时，可以使用"搜索和还原"从 ONTAP 备份中恢复卷、文件夹或文件。您可以搜索本地快照、已复制卷和对象存储备份。
...
* 当您还原启用了 SnapLock 的卷时，只有当目标存储启用了 WORM 时，还原的卷才会保留 SnapLock 保护。如果还原到非 WORM 目标，则会从还原的卷中删除 SnapLock 状态。


====
在开始之前，应了解要还原的卷或文件的名称或位置。
...
| StorageGRID | 本地ONTAP系统Cloud Volumes ONTAP | 本地ONTAP系统 
...
** 必须在存储虚拟机上配置 IPv4 NFS 数据 LIF，并为卷配置 NFS 导出。Search & Restore 支持 NFS 和 SMB/CIFS 卷。
** 必须在存储 VM 上激活 SnapDiff RPC Server。在系统上启用索引时，Console 会自动执行此操作。（SnapDiff 是一种可快速识别快照之间文件和目录差异的技术。）
...
NOTE: Backup and Recovery 不会为创建时间戳与同一卷的另一个快照完全匹配的任何快照编制索引。
...
首先，装载一个单独的卷以保存目录文件。这有助于在快照目录文件过大时防止数据丢失。这不是每个集群都需要的；您可以从环境中的任何集群装载任意一个卷。如果不这样做，索引可能无法正常工作。
...
.. 在_筛选条件_区域中，选择筛选条件。例如，您可以选择数据所在的系统和文件类型，例如 .JPEG 文件。如果要仅在对象存储中的可用快照或备份文件中进行搜索，也可以选择备份位置类型。
...
+
请注意，结果会识别包含搜索中文件的本地卷快照和远程复制卷。您可以选择从云备份文件、快照或复制的卷中恢复。
...
+
*** 从 Amazon S3 还原时，请选择目标卷所在的 ONTAP 集群中的 IPspace，输入您创建的用于授予 ONTAP 集群访问 S3 存储桶权限的用户的访问密钥和密钥，并可选择用于安全数据传输的私有 VPC 端点。如果使用私有 VPC 端点，则它会优先于任何已配置的代理连接。link:prev-ontap-backup-onprem-aws.html["查看有关这些要求的详细信息"]。
