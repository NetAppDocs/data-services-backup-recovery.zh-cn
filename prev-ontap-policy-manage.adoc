...
NetApp Backup and Recovery 提供三种类型的ONTAP数据备份：快照、复制和对象存储备份。根据您使用的架构和备份类型，它们的策略位于不同的位置：
...
. 指定策略名称。
. 选择一个或多个快照计划。您最多可以有 5 个标签。或者，创建一个时间表。
. 如果您选择创建时间表：
+
...
您的 3-2-1 策略可能包括在不同的存储系统上复制卷。复制策略位于*辅助*存储系统上。
