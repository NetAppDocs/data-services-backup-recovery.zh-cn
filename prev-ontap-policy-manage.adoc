...
[role="lead"]
在 NetApp Backup and Recovery 中，您可以使用默认备份策略或为您的 ONTAP 卷创建自定义策略。策略控制备份频率、备份时间和保留。


NOTE: 要切换到NetApp Backup and Recovery工作负载，请参阅link:br-start-switch-ui.html["切换到不同的NetApp Backup and Recovery工作负载"]。

使用激活向导为卷启用备份和恢复服务时，可以从默认策略和系统中已存在的任何其他策略（Cloud Volumes ONTAP 或本地 ONTAP 系统）中进行选择。如果要使用与现有策略不同的策略，可以在使用激活向导之前或期间创建策略。

要了解提供的默认备份策略，请参阅link:prev-ontap-protect-journey.html["规划您的保护之旅"]。

NetApp Backup and Recovery 提供三种类型的 ONTAP 数据备份：快照、复制和备份到对象存储。根据您使用的架构和备份类型，其策略驻留在不同的位置：
