...
[role="lead"]
要提高 NetApp Console 本地部署中的搜索和还原性能，请将目录索引存储从集群的默认存储类迁移到 ONTAP 支持的 NFS 共享。目录索引支持搜索和还原，而 ONTAP 支持的 NFS 可以提供可衡量的性能改进。
...
[discrete]
===== 在 NetApp Console 本地部署上挂载 ONTAP 卷并从 Pod 复制数据
...
----
. 先应用 PV，然后再应用 PVC，以此应用 YAML 对象（PV 必须存在，PVC 的标签选择器才能绑定到该对象）。然后验证 PVC 声明是否已绑定：
+
[source, shell]
----
