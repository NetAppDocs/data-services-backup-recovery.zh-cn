...
---
= 在 Backup and Recovery 仪表板上查看保护运行状况
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
监控工作负载的运行状况，以识别保护问题并响应警报。查看备份和还原的状态，并查看 NetApp Backup and Recovery 仪表板上的系统、保护、作业和还原摘要。

*必需的 NetApp Console 角色* Backup and Recovery 超级管理员、Backup and Recovery 备份管理员、Backup and Recovery 恢复管理员、Backup and Recovery 克隆管理员或 Backup and Recovery 备份查看器角色。如果要保护存储在 Cloud Volumes ONTAP 集群上的数据，则还需要 Cloud Volumes ONTAP 管理员角色。了解 link:reference-roles.html["备份和恢复角色和权限"]。 https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["了解所有服务的NetApp Console访问角色"^]。
...
.步骤
. 对于每个作业分配，更改筛选器以根据天数显示失败、正在运行和已完成作业的摘要，例如，过去 30 天、过去 7 天、过去 24 小时或过去一年。
. 选择*查看作业监控*，查看失败、正在运行和已完成作业的详细信息。
