...
---
= 使用 NetApp Backup and Recovery 将 Cloud Volumes ONTAP 备份到 Google Cloud Storage
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用 NetApp Backup and Recovery 将卷数据从 Cloud Volumes ONTAP 系统备份到 Google Cloud Storage。
...
您还需要一个 Google Cloud 帐户，用于存储备份所在的存储空间。
...
要使用 NetApp Backup and Recovery 搜索和还原功能，Console 代理的角色必须包含特定权限，以便可以访问 Google Cloud BigQuery 服务。请参阅以下权限，如果您需要修改策略，请按照以下步骤操作。
...
. *详细信息和凭证*：输入以下信息：
+
.. 如果要使用的项目与默认项目（Console 代理驻留的项目）不同，请选择 *Edit Project* 并选择新项目。
.. 指定集群名称。
...
. *服务*：保持 NetApp Backup and Recovery 处于启用状态，然后选择*继续*。
...
.. 在 *Access keys for service accounts* 下，选择 *Create a key for a service account*，选择刚刚创建的服务帐户，然后选择 *Create Key*。
+
...
+
如果备份的 Google Cloud Storage 目标在 Console *Systems* 页面上作为系统存在，则可以将 ONTAP 集群拖动到 Google Cloud Storage 系统上。
...
** 输入策略的名称。
** 选择最多五个时间表，通常频率不同。
** 对于备份到对象策略，请配置 Datalock 和 Ransomware 保护。有关 Datalock 和 Ransomware 保护的详细信息，请参阅 link:prev-ontap-policy-object-options.html["备份到对象策略设置"]。
** 选择“*创建*”。
