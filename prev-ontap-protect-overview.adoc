...
---
= 了解使用 NetApp Backup and Recovery 的 ONTAP 卷保护
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用 NetApp Backup and Recovery 来保护和恢复您的 ONTAP 卷数据，并长期存档备份。您可以实施 3-2-1 策略，在两个不同的存储系统上保留三个源数据副本，并在云端保留一个副本。


NOTE: 要切换到NetApp Backup and Recovery工作负载，请参阅link:br-start-switch-ui.html["切换到不同的NetApp Backup and Recovery工作负载"]。

激活后，Backup and Recovery 会创建块级增量永久备份，这些备份存储在另一个 ONTAP 集群和云中的对象存储中。除了源卷之外，您还将拥有：
...
NetApp Backup and Recovery 使用 NetApp 的 SnapMirror 数据复制技术，通过创建快照并将其传输到备份位置来保持备份同步。
...
* 通过在两个系统之间使用预共享密钥 (PSK) 设置本机 ONTAP 传输中加密。
* 复制的数据是不可变的，直到您使其可写并可供使用为止。
* 如果传输失败，复制可以自我修复。
* 与 https://docs.netapp.com/us-en/data-services-replication/index.html["NetApp Replication"^]相比，NetApp Backup and Recovery 中的复制具有以下特点：
+
...
* 扫描备份文件以查找可能的勒索软件攻击，并自动删除或替换受感染的备份。
...
NetApp Backup and Recovery 使您能够将 ONTAP 卷从以下源系统备份到公共和私有云提供商中的以下辅助系统和对象存储。快照驻留在源系统上。


NOTE: Wasabi 目前是唯一符合条件的 S3 兼容对象存储提供商。其他 S3 兼容的提供商尚未通过验证。
...
| S3 兼容对象存储 | 本地ONTAP系统 | 本地ONTAP系统 
|===

NOTE: "本地 ONTAP 系统"包括 FAS、AFF 和 ONTAP Select 系统。
...
* 如果您计划在恢复卷数据的过程中扫描备份文件中的勒索软件（如果您已为云备份启用了 DataLock 和勒索软件保护），那么您还将产生云提供商的额外出口费用。
...
如果您的集群使用的是 ONTAP 9.11.1 或更高版本，您可以保护对象存储中的备份免受删除和勒索软件攻击。每个备份策略都提供了一个关于 _DataLock and Ransomware protection_ 的部分，可在特定时间段（即_保留期_）内应用于您的备份文件。
...
当备份目标为 Amazon S3、Azure Blob 或 StorageGRID 时，当前支持此功能。其他存储提供商目标将在未来版本中添加。
...
+
如果您的本地集群使用的是 ONTAP 9.12.1 或更高版本，并且您的 StorageGRID 系统使用的是 11.4 或更高版本，则可以在一定天数后将较旧的备份文件归档到公共云归档存储中。当前支持 AWS S3 Glacier/S3 Glacier Deep Archive 或 Azure Archive 存储层。link:prev-ontap-backup-onprem-storagegrid.html["了解有关从StorageGRID归档备份文件的更多信息"]。



请参阅link:prev-ontap-policy-object-options.html["存档旧备份文件的工作原理"]。
