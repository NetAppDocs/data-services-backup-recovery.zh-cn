...
---
= 在 NetApp Backup and Recovery 中添加和保护 Kubernetes 应用程序
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用 NetApp Backup and Recovery Web UI 或自定义资源文件添加 Kubernetes 应用程序以进行保护。您可以添加使用标准 Kubernetes 资源的基于命名空间的应用程序，也可以添加使用一个或多个虚拟机的基于虚拟机的应用程序。
...
要保护 Kubernetes 应用程序，请先在 NetApp Backup and Recovery 中创建应用程序。当您创建应用程序时，NetApp Backup and Recovery 将识别 Kubernetes 集群上正在运行的应用程序。
...
. 在页面右上角，确保在工作负载列表中选择了 *Kubernetes*。
. 选择工作负载行项目的*View*以查看 Kubernetes 资源。
. 选择“*应用程序*”选项卡。
. 选择*创建应用程序*。
. 输入应用程序的名称。
...
NOTE: Backup and Recovery 不存储搜索参数或结果；这些参数用于搜索所选 Kubernetes 集群中可以包含在应用程序中的资源。
...
. （可选）在 *Policy* 区域中，选择一个现有的保护策略来保护应用程序或创建一个新策略。如果未选择策略，则创建的应用程序将没有保护策略。您可以link:br-use-policies-kubernetes-create.html#create-a-policy["添加保护策略"]稍后进行。
...
NOTE: 如果从列表中选择虚拟机，则应用程序定义为静态——稍后不会将新虚拟机添加到应用程序（您需要编辑应用程序以添加和保护它们）。如果使用标签选择器，则无法选择单个虚拟机或编辑生成的列表，但后来与选择器匹配的任何虚拟机都会自动包含在内并受到保护。
...
.结果
应用程序已创建，并显示在 Kubernetes 清单的*应用程序*选项卡中的应用程序列表中。NetApp Backup and Recovery 可根据您的设置为应用程序提供保护，您可以在*监控*区域中监控进度。
...
.. 如果需要，请配置以下注释：
+
*** *metadata.annotations.protect.trident.netapp.io/skip-vm-freeze*：（_Optional_）此注释仅适用于从虚拟机定义的应用程序，例如在 KubeVirt 环境中，在快照之前会发生文件系统冻结。指定此应用程序是否可以在快照期间写入文件系统。如果设置为 true，应用程序将忽略全局设置，并且可以在快照期间写入文件系统。如果设置为 false，应用程序将忽略全局设置，并在快照期间冻结文件系统。如果指定，但应用程序在应用程序定义中没有虚拟机，则忽略注释。如果未指定，则应用程序遵循 https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-requirements.html#protecting-data-with-kubevirt-vms["全局文件系统冻结设置"]。
