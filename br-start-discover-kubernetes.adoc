...
. （可选）选择一个文件夹或项目，以确定访问控制应用于此集群的位置。如果未选择文件夹或项目，则会在组织级别添加集群 IAM 资源：
+
** *Folder*：选择此选项可控制用户对文件夹级别群集资源的访问，并从列表中选择一个文件夹。
+

NOTE: 如果 Kubernetes 集群或命名空间与文件夹关联，则用户还需要在右上角的项目选择器中所选项目的 *Backup and Recovery viewer* 角色。此项目级访问权限使用户能够打开 Backup and Recovery 主页。在文件夹级别分配的角色（例如，Backup and Recovery backup admin）确定用户进入工作负载页面后可以执行的操作，但不会自行授予对该页面的访问权限。请参见 link:br-use-manage-kubernetes-access-control.html["管理 Kubernetes 资源访问"] 以了解更多信息。

** *Project*：选择此选项可在项目级别控制用户对集群资源的访问，并从列表中选择一个项目。
+
...
. 选择 *Generate installation commands*。
+
...
+
** 选择*已连接*以发现具有外部网络连接的集群。这些说明是针对已连接集群量身定制的。
** 选择 *Air-gapped* 以发现没有外部网络连接的集群。这些说明专为气隙（离线）集群定制。
+
[role="tabbed-block"]
====
.已连接发现
