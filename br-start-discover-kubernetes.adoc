...
在备份和恢复清单中，发现环境中的 Kubernetes 工作负载。添加工作负载会将 Kubernetes 集群添加到 NetApp Backup and Recovery。然后，您可以添加应用程序并保护集群资源。您可以选择使用 NetApp Console 本地部署来发现气隙本地 Kubernetes 资源。
...
+
备份和恢复提供根据所选安装类型而变化的说明。如果使用 NetApp Console 本地部署，则可以选择相应说明，以发现没有外部网络连接的气隙资源。
