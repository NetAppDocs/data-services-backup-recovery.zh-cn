...
[role="lead"]
使用自定义资源从快照或备份还原应用程序。将应用程序还原到同一集群时，从现有快照还原的速度通常更快。
...
* 您可以使用高级设置自定义还原操作。如需了解更多信息，请参见 link:br-use-kubernetes-advanced-restore-settings.html["NetApp Backup and Recovery 中 Kubernetes 的高级还原设置"]。
...
* 当您使用 CR 还原到新的命名空间时，您必须在应用 CR 之前手动创建目标命名空间。仅当您使用 CLI 时，备份和恢复才会自动创建命名空间。
...
NOTE: 如果 BackupRestore CR 不可用，您可以使用步骤 2 中的命令查看备份内容。
