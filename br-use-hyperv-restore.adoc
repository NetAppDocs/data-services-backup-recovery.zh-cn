---
sidebar: sidebar 
permalink: br-use-hyperv-restore.html 
keywords: restore, vm, virtual machine, snapshots, object storage, cloud storage, backing up, back up, backup, Amazon Web services, AWS, NetApp Backup and Recovery 
summary: 使用NetApp Backup and Recovery保护您的 Hyper-V 工作负载。 
---
= 使用NetApp Backup and Recovery恢复 Hyper-V 工作负载
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/


[role="lead"]
使用 NetApp Backup and Recovery 从本地快照、辅助存储上的复制备份或对象存储中的备份还原 Hyper-V 工作负载。

.从这些位置恢复
您可以从以下位置还原工作负载：

* 从主位置恢复（本地快照）
* 从二级存储上的复制资源恢复
* 从对象存储备份恢复


.恢复到这些点
您可以将数据还原到以下目的地：

* 还原到原始位置（从主存储、辅助存储和对象存储）
* 还原到备用位置（从主、辅助和对象存储）


.从对象存储中恢复的注意事项
如果在对象存储中选择一个备份文件，并且由于您在备份策略中启用了 DataLock 和勒索软件保护，因此该备份的勒索软件保护处于活动状态，则在还原数据之前，系统会提示您运行其他完整性检查。我们建议您执行扫描。


TIP: 您将需要向云提供商支付额外的出口费用才能访问备份文件的内容。



[[how-restoring-workloads-works]]
== 恢复工作负载的工作原理

还原工作负载时，将发生以下情况：

* 当您从本地备份文件恢复工作负载时， NetApp Backup and Recovery会使用备份中的数据创建一个_新_资源。
* 从复制的工作负载恢复时，您可以将工作负载恢复到原始系统或本地ONTAP系统。


从还原页面，即使不记得资源的确切名称、位置或上次正常备份的日期，也可以还原资源。您可以使用筛选器搜索快照。



[[restore-hyper-v-workloads]]
== 恢复 Hyper-V 工作负载

使用"还原"菜单还原 Hyper-V 工作负载。您可以按名称或使用过滤器搜索快照。

*必需的 NetApp Console 角色* 备份和恢复超级管理员或备份和恢复还原管理员角色 https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["了解所有服务的NetApp Console访问角色"^]。

.步骤
. 从NetApp Backup and Recovery菜单中，选择 *恢复*。
. 从名称搜索字段右侧的下拉列表中，选择 *Hyper-V*。
. 输入要还原的资源的名称，或按 VM 名称、VM 主机或资源所在的存储池进行筛选。
+
出现符合您的搜索条件的快照列表。

. 选择要恢复的快照的“*恢复*”按钮。
+
出现可能的还原点列表。

. 选择您想要使用的还原点。
. 选择快照源位置。
. 选择“下一步”继续。
. 选择恢复目标位置和设置：
+
[discrete]
===== 目的地选择

+
[role="tabbed-block"]
====
.恢复到原始位置
--
.. 选择 *Original location* 窗格。当您还原到原始位置时，您可以通过展开 *Destination settings* 部分查看目标设置，但不能更改它们。
.. 在“恢复后选项”部分，请考虑以下选项：
+
*** *启动虚拟机*：启用此选项可在恢复后启动新的虚拟机。




--
.恢复至备用位置
--
.. 选择“备用位置”窗格。
.. 在*目标设置*部分中，输入以下信息：
+
*** *Hyper-V FQDN 或 IP 地址*：输入目标 Hyper-V 主机的完全限定域名或 IP 地址。
*** *网络*：选择要将快照还原到的目标网络。
*** *虚拟机名称*：输入要还原的虚拟机的名称。
*** *目标位置*：输入应包含已恢复数据的目标文件夹或 CIFS 共享。


.. 在*目标存储*部分（仅在从对象存储还原时可见）：
+
*** *更改默认存储位置*：如果源存储位置无法访问或已满，则启用此选项可将对象存储备份还原到其他位置。选择要使用的其他集群、存储 VM 或聚合。


.. 在*预还原选项*部分中，请考虑以下选项：
+
*** *快速恢复*：启用此选项可使恢复的虚拟机立即可用。仅从对象存储中恢复运行虚拟机所需的文件，而不是整个卷。


.. 在*还原后选项*部分中，请考虑以下选项：
+
*** *启动虚拟机*：启用此选项可在恢复后启动新的虚拟机。




--
====
. 选择*恢复*。

