...
[role="lead"]
使用此页面可验证您的配置是否受支持，并在 NetApp Backup and Recovery 中将备份从主本地 ONTAP 系统配置到与 S3 兼容的对象存储。
...
.步骤
. 使用以下方法之一访问"激活备份和恢复"向导：
+
** 在 Console *Systems* 页面中，选择系统，然后选择右侧面板中 Backup and Recovery 旁边的 *Enable > Backup Volumes*。
...
====
.步骤
请注意，如果您选择的卷已经应用了快照或复制策略，则稍后选择的策略将覆盖这些现有策略。
...
TIP: 如果要在激活快照之前创建自定义策略，可以使用 System Manager 或 ONTAP CLI `snapmirror policy create`命令。请参阅。
...
*** 输入策略的名称。
*** 选择最多五个时间表，通常频率不同。
*** 对于备份到对象策略，请设置 DataLock 和勒索软件防护设置。有关 DataLock 和勒索软件防护的详细信息，请参阅 link:prev-ontap-policy-object-options.html["备份到对象策略设置"]。
*** 选择“*创建*”。


** *将现有快照作为备份文件导出到对象存储*：如果此系统中的卷有任何本地快照与您刚刚选择的备份计划标签（例如每天、每周等）匹配，则会显示此附加提示。选中此框可将所有历史快照作为备份文件复制到对象存储，以确保对卷进行最完整的保护。
...
.步骤
. 在“审核”页面中，审核您的选择。
. （可选）选中该框以*自动将快照策略标签与复制和备份策略标签同步*。这将创建带有与复制和备份策略中的标签相匹配的标签的快照。如果策略不匹配，则不会创建备份。
. 选择*激活备份*。
...
您可能需要显示并（可选）复制"激活备份和恢复"向导中使用的 API 命令。您可能需要这样做，以便在未来的系统中自动激活备份。
