...
.步骤
. 在“卷”选项卡中，找到要激活备份的卷，然后选择“操作”菜单。 image:icon-action.png["操作图标"] 在行尾，选择“激活 3-2-1 保护”。
...
+
如果在激活此群集的 NetApp Backup and Recovery 时，在初始备份策略中为云备份启用了_数据锁和勒索软件保护_，则只能看到已配置 DataLock 的其他策略。如果您在激活 NetApp Backup and Recovery 时没有启用_数据锁和勒索软件保护_，您只会看到其他未配置 DataLock 的云备份策略。
...
如果在激活此集群的 NetApp Backup and Recovery 时启用了_Datalock 和 Ransomware 保护_，则按需备份也将配置 DataLock，保留期为 30 天。临时备份不支持勒索软件扫描。link:prev-ontap-policy-object-options.html["了解有关 DataLock 和勒索软件保护的更多信息"^]。
...
仅当卷备份是从运行 ONTAP 9.11.1 或更高版本的系统创建的，并且您在备份到对象策略中启用了_数据锁和勒索软件保护_时，此功能才可用。
...
[NOTE]
====
* 如果在为此群集激活 NetApp Backup and Recovery 时在初始策略中启用了_数据锁和勒索软件保护_，则必须使用相同的 DataLock 设置（治理或合规性）配置您编辑的任何策略。如果您在激活 NetApp Backup and Recovery 时没有启用_数据锁和勒索软件保护_，则您现在无法启用 DataLock。
...
[NOTE]
====
* 如果在为此群集激活 NetApp Backup and Recovery 时在初始策略中启用了_数据锁和勒索软件保护_，则必须使用相同的 DataLock 设置（治理或合规性）配置您创建的任何其他策略。如果您在激活 NetApp Backup and Recovery 时未启用_数据锁和勒索软件保护_，则无法创建使用 DataLock 的新策略。
...
.步骤
. 从 *Volumes* 菜单中，单击源卷的 image:icon-actions-horizontal.gif["更多图标"]，然后选择 *Manage backups > View backup*。
+
显示所有备份文件的列表。
