...
[role="lead"]
使用 NetApp Backup and Recovery 管理 Cloud Volumes ONTAP 和本地 ONTAP 系统的快照、复制和对象存储备份。您可以更改计划、启用或禁用卷备份、暂停备份、删除备份以及取消注册服务。
...
. 查看"现有保护"列中保护选项的状态。这三个图标代表"本地快照"、"已复制卷"和"对象存储中的备份"。
+
...
有关说明，请参阅 link:br-start-enable.html["启用 NetApp Backup and Recovery 以保护卷"^]。
...
.步骤
. 从“卷”菜单中，找到要修改策略设置的卷，然后选择“操作”菜单。 image:icon-action.png["操作图标"] 在行尾，选择“编辑备份策略”。
. 在“编辑备份策略”页面中，对本地快照、复制卷和备份文件的现有备份策略进行更改，然后选择“下一步”。
+
如果您在为此集群激活 NetApp Backup and Recovery 时，在初始备份策略中为云备份启用了 _DataLock 和勒索软件保护_，则您将只能看到其他配置了 DataLock 的策略。如果您在激活 NetApp Backup and Recovery 时未启用 _DataLock 和勒索软件保护_，则您将只能看到其他未配置 DataLock 的云备份策略。
...
如果在激活此集群的 NetApp Backup and Recovery 时启用了 _DataLock 和勒索软件保护_，则按需备份也将配置 DataLock，保留期为 30 天。临时备份不支持勒索软件扫描。link:prev-ontap-policy-object-options.html["了解有关 DataLock 和勒索软件保护的更多信息"^]。

创建临时备份时，将在源卷上创建一个快照。由于此快照不是正常快照计划的一部分，因此不会自动轮换删除。备份完成后，您可能需要从源卷中手动删除此快照。这将释放与此快照相关的块。快照名称将以 `cbs-snapshot-adhoc-`开头。 https://docs.netapp.com/us-en/ontap/san-admin/delete-all-existing-snapshot-copies-volume-task.html["请参阅如何使用 ONTAP CLI 删除快照"^]。
...
仅当卷备份是从运行 ONTAP 9.11.1 或更高版本的系统创建的，且在备份到对象策略中启用了 _DataLock 和勒索软件保护_ 时，此功能才可用。
...
. 选择要扫描勒索软件的卷备份文件对应的 image:icon-actions-horizontal.gif["操作图标"]，然后选择 *扫描勒索软件*。
+
勒索软件防护列显示扫描正在进行中。
...
[NOTE]
====
* 如果在为此集群激活 NetApp Backup and Recovery 时在初始策略中启用了 _DataLock and Ransomware protection_，则您编辑的任何策略都必须配置相同的 DataLock 设置（治理或合规性）。如果在激活 NetApp Backup and Recovery 时未启用 _DataLock and Ransomware protection_，则现在无法启用 DataLock。
...
+
link:prev-reference-aws-archive-storage-tiers.html["了解有关使用 AWS 档案存储的更多信息"]。link:prev-reference-azure-archive-storage-tiers.html["了解有关使用 Azure 档案存储的详细信息"]。link:prev-reference-gcp-archive-storage-tiers.html["详细了解如何使用 Google 归档存储"]。（需要ONTAP 9.12.1。）
...
[NOTE]
====
* 如果在为此集群激活 NetApp Backup and Recovery 时在初始策略中启用了 _DataLock and Ransomware protection_，则您创建的任何其他策略都必须配置相同的 DataLock 设置（Governance 或 Compliance）。如果在激活 NetApp Backup and Recovery 时未启用 _DataLock and Ransomware protection_，则无法创建使用 DataLock 的新策略。
...
NOTE: 删除备份关系后，无法删除单个卷备份文件。在删除卷的备份关系之前，请先删除卷的备份文件。

.步骤
. 在*卷*菜单中，为源卷选择 image:icon-actions-horizontal.gif["更多图标"]，然后选择*管理备份 > 查看备份*。
+
显示所有备份文件的列表。

. 从列表中选择一个备份，然后选择 *Actions > Delete all Backups*。
. 通过输入卷名称确认删除。
. 展开 *高级设置* 区域。
. 在 *强制删除备份* 下，选择是否强制删除所有备份。
+
在极少数情况下，您可能需要撤销 NetApp Backup and Recovery 对备份的访问权限；例如，如果 Backup and Recovery 失去对备份存储区的访问权限，或者备份受到 DataLock 保护，但您不再需要它们。使用此选项可在卷和系统级别强制删除备份。

+

CAUTION: 此选项仅用于极端清理。启用后，NetApp Backup and Recovery 将不再访问这些备份（即使对象仍保留在云存储中）。您必须从云提供商手动删除它们。
...
当不再需要单个备份时，请将其删除。您无法删除已复制卷（数据保护卷）。


NOTE: 删除备份关系后，无法删除单个卷备份文件。但是，您可以<<删除卷的所有备份文件,删除此卷的所有备份文件>>。
...
NOTE: 删除备份关系后，无法删除单个卷备份文件。但是，可以<<删除卷的所有备份文件,删除此卷的所有备份文件>>。如果需要删除卷的备份关系，请确保首先删除卷的备份，然后删除备份关系。
...
停用系统的 NetApp Backup and Recovery 会暂停其所有卷的备份和还原，而不会删除现有备份。这使您能够在一段时间内暂停所有备份和还原活动，而无需从此系统注销备份服务。
