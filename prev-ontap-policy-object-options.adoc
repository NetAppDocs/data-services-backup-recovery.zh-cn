...
这两项功能都可以保护您的备份文件，以便在备份遭到勒索软件攻击时，您始终拥有有效的备份文件来恢复数据。在备份需要锁定和保留一段时间的情况下，满足某些监管要求也很有帮助。启用 *Datalock and Ransomware protection* 选项后，作为 NetApp Backup and Recovery 激活的一部分配置的云存储桶将启用对象锁定和对象版本控制。
...
. 创建策略时，展开 *Datalock 和 Ransomware 防护* 部分。
. 选择下列选项之一：
+
