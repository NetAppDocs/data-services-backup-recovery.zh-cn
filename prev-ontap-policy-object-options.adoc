...
[role="lead"]
在 NetApp Backup and Recovery 中使用备份到对象策略，为您的本地 ONTAP 和 Cloud Volumes ONTAP 系统定义计划、保留、DataLock、勒索软件保护和归档设置。
...
每个备份策略都为_Labels & Retention_提供了一个部分，您可以将其应用于备份文件。请注意，应用于卷的快照策略必须为 NetApp Backup and Recovery 所识别的策略之一，否则将不会创建备份文件。
...
这两项功能都可以保护您的备份文件，从而确保在您的备份遭遇勒索软件攻击时，您始终拥有一个有效的备份文件来恢复数据。这也有助于满足某些监管要求，即需要将备份锁定并保留一段时间。启用 *DataLock and Ransomware protection* 选项后，作为 NetApp Backup and Recovery 激活的一部分而配置的云存储桶将启用对象锁定和对象版本控制。
...
. 创建策略时，展开 *DataLock 和勒索软件防护* 部分。
. 选择下列选项之一：
+
** *无*：已禁用 DataLock 保护和勒索软件保护。
** *已解锁*：已启用 DataLock 保护和勒索软件保护。具有特定权限的用户可以在保留期间覆盖或删除受保护的备份文件。
** *已锁定*：已启用 DataLock 保护和勒索软件保护。在保留期间，任何用户都不能覆盖或删除受保护的备份文件。这满足了完全的法规遵从性。
...
| Google Cloud 中的Cloud Volumes ONTAP | Google Cloud Storage 
...
使用 AWS、Azure 或 Google Cloud Storage 时，您可以在一定天数后将较旧的备份文件移至成本较低的归档存储类或访问层。您还可以选择立即将备份文件发送到归档存储，而无需写入标准云存储。只需输入 *0* 作为"Archive After Days"，即可将备份文件直接发送到归档存储。这对于很少需要从云备份访问数据的用户或正在替换磁带备份解决方案的用户尤其有用。
