...
---
= 使用 NetApp Backup and Recovery 将 Cloud Volumes ONTAP 备份到 Amazon S3
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
使用 NetApp Backup and Recovery 将卷数据从 Cloud Volumes ONTAP 系统备份到 Amazon S3。
...
NOTE: 如果使用专用 VPC 端点进行备份或还原数据传输，则会优先于任何已配置的代理连接。
...
.步骤
. 在 Console *Systems* 页面中，选择集群，然后选择右侧面板中 Backup and Recovery 旁边的 *Enable*。
+
如果备份的 Amazon S3 目标在*系统*页面上作为系统存在，则可以将集群拖动到 Amazon S3 系统上以启动设置向导。
...
+
如果备份的 Amazon S3 目标在 Console *Systems* 页面上作为系统存在，则可以将 ONTAP 集群拖动到 Amazon S3 系统上。

** 在 Backup and Recovery 栏中选择*卷*。从卷选项卡中，选择*操作* image:icon-action.png["操作图标"] 图标，然后为单个卷（尚未启用复制或备份到对象存储）选择*激活 3-2-1 保护*。
...
.步骤
. 在“审核”页面中，审核您的选择。
. （可选）选中该框以*自动修复本地快照、复制和备份上的不匹配标签*。这将创建具有与快照、复制和备份策略中的标签相匹配的标签的快照。
. 选择*激活备份*。
