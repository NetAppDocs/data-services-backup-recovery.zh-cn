...
summary: 使用 NetApp Backup and Recovery，通过浏览和还原功能还原 ONTAP 数据。 
---
= 使用 NetApp Backup and Recovery 中的浏览和还原功能还原 ONTAP 数据
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
当您知道源卷名、源系统、存储 VM 和备份文件日期时，请使用"浏览和还原"。您可以从快照、复制卷或存储在对象存储中的备份中还原 ONTAP 数据。
...
NOTE: 将数据从 Azure 存档存储还原到 StorageGRID 系统时，不支持高还原优先级。
...
您可以将驻留在辅助系统（复制卷）或对象存储（备份文件）中的备份文件中的 ONTAP 数据还原到以下系统。
...
* 对于Amazon S3，可以在AWS或本地部署Console代理
* 对于 Azure Blob，可以在 Azure 中或本地部署 Console 代理
* 对于 Google Cloud Storage，控制台代理必须部署在您的 Google Cloud Platform VPC 中
* 对于 StorageGRID，必须在本地部署 Console 代理，无论是否接入互联网
* 对于 ONTAP S3，Console 代理可以部署在内部（有或没有互联网访问）或云提供商环境中


请注意，对"本地 ONTAP 系统"的引用包括 FAS、AFF 和 ONTAP Select 系统。
...
当使用 ONTAP 9.13.0 或更高版本将云备份还原到 Cloud Volumes ONTAP 系统，或还原到运行 ONTAP 9.14.1 的内部部署 ONTAP 系统时，您可以选择执行_快速还原_操作。快速还原非常适合需要尽快提供卷访问权限的灾难恢复情况。快速还原会将元数据从备份文件还原到卷，而不是还原整个备份文件。对于性能或延迟敏感的应用程序，不建议使用快速还原，并且存档存储中的备份不支持快速还原。


NOTE: 仅当创建云备份的源系统运行 ONTAP 9.12.1 或更高版本时，才支持对 FlexGroup 卷进行快速恢复。仅当源系统运行 ONTAP 9.11.0 或更高版本时，才支持对 SnapLock 卷进行快速恢复。
...
.步骤
. 从控制台菜单中，选择*保护>备份和恢复*。
. 选择 *Restore* 选项卡以显示 Restore Dashboard。
. 从“浏览和恢复”部分，选择“恢复卷”。
. 在“选择源”页面中，导航到要恢复的卷的备份文件。选择具有要恢复的日期/时间戳的*系统*、*卷*和*备份*文件。
+
*位置*列显示备份文件（快照）是*本地*（源系统上的快照）、*辅助*（辅助 ONTAP 系统上的复制卷）还是*对象存储*（对象存储中的备份文件）。选择要还原的文件。

. 选择“下一步”。
+
请注意，如果您在对象存储中选择一个备份文件，并且该备份的勒索软件保护处于活动状态（如果您在备份策略中启用了 DataLock 和勒索软件保护），系统会提示您在恢复数据之前对备份文件运行额外的勒索软件扫描。我们建议您扫描备份文件以查找勒索软件。（访问备份文件内容将产生云提供商的额外出口费用。）

. 在“选择目标”页面中，选择要恢复卷的*系统*。
. 从对象存储还原备份文件时，如果您选择本地ONTAP系统并且尚未配置与对象存储的集群连接，系统将提示您输入其他信息：
+
** 从 Amazon S3 还原时，请选择目标卷所在的 ONTAP 集群中的 IP 空间，输入您创建的用户的访问密钥和机密密钥，以授予 ONTAP 集群对 S3 存储桶的访问权限，并可选择用于安全数据传输的私有 VPC 端点。如果使用专用 VPC 端点，则它会优先于任何已配置的代理连接。
...
. （可选）启用 *Prioritize this restore over existing job queue* 以赋予此还原操作更高的优先级。启用后，其他正在运行的作业可能会暂停或延迟，以便此还原操作继续进行。
. 选择*恢复*。
+
您将返回"还原控制面板"，以便查看还原操作的进度。
...
* 使用 ONTAP 9.15.1，您可以使用"浏览和还原"选项还原 FlexGroup 文件夹。
+
...
+
如果备份文件的勒索软件保护处于活动状态（如果您在备份策略中启用了 DataLock 和勒索软件保护），系统会提示您在还原数据之前对备份文件运行额外的勒索软件扫描。我们建议您扫描备份文件以查找勒索软件。（访问备份文件内容将产生云提供商的额外出口费用。）
...
+
*** 您可以选择任意文件夹。
*** 您可以将鼠标悬停在文件夹上，选择行末尾以深入到子文件夹，然后选择一个文件夹。


** 如果您选择了与源文件夹或文件位置相同的目标系统和卷，则可以选择 *维护源文件夹路径* 将文件夹或文件还原到源结构中存在的相同文件夹。所有相同的文件夹和子文件夹必须已存在；系统不会创建文件夹。将文件还原到其原始位置时，可以选择覆盖源文件或创建新文件。
