...
[role="lead"]
使用 NetApp Backup and Recovery，通过“浏览并恢复”来恢复 ONTAP 数据。在恢复之前，请记录源卷名称、源系统和 Storage VM 以及备份文件日期。您可以从 Snapshot、复制的卷或存储在对象存储中的备份中恢复 ONTAP 数据。
...
. 选择“下一步”。
+
请注意，如果您在对象存储中选择一个备份文件，并且该备份的勒索软件保护处于活动状态（如果您在备份策略中启用了 Datalock 和勒索软件保护），则在还原数据之前，系统会提示您对备份文件运行其他勒索软件扫描。我们建议您扫描备份文件以查找勒索软件。（为了访问备份文件的内容，您的云提供商将产生额外的出口成本。）
...
+
如果备份文件的勒索软件保护处于活动状态（如果您在备份策略中启用了 Datalock 和勒索软件保护），则在还原数据之前，系统会提示您对备份文件运行其他勒索软件扫描。我们建议您扫描备份文件以查找勒索软件。（为了访问备份文件的内容，您的云提供商将产生额外的出口成本。）
